...
function gen_auth_token(email)
	...
	log('AUTH-TOKEN', 'usr=%s token=%s'..(ok and '' or ' error=%s'), usr, token, err)
...
function gen_auth_code(validates, s)
	...
	log('AUTH-CODE', 'usr=%s code=%s validates=%s'..(ok and '' or ' error=%s'),
...
local function facebook_graph_request(url, args)
	...
	log('FACEBOOK', 'facebook_graph_request: %s %s -> %s',
...
local function google_api_request(url, args)
	...
	log('GOOGLE', 'google_api_request: %s %s -> %s',
		url, pp.format(args, ' '), pp.format(res, ' '))
end
